        ...
        <div className="page-container space-y-4">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="mt-0.5 text-[10px] font-black uppercase tracking-widest text-black/40 dark:text-white/40">Collaborate & Compete</p>
                </div>
                <Link
                    href="/dashboard/squads/new"
                    className="flex items-center justify-center gap-1.5 rounded-lg bg-primary-600 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-white shadow-lg transition-all hover:bg-primary-700"
                >
            ...
            <div className="grid gap-3 md:grid-cols-12">
                <div className="md:col-span-8">
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className="w-full rounded-xl border border-surface-200/50 bg-white/50 py-2 pl-9 pr-4 text-[12px] font-bold outline-none backdrop-blur-sm transition-all focus:border-primary-500/50 dark:border-white/[0.05] dark:bg-white/[0.02]"
                        />
                    </div>
                </div>
                <div className="md:col-span-4 flex gap-3">
                    <div className="flex-1 rounded-xl border border-surface-200/50 bg-white/50 p-2 dark:border-white/[0.05] dark:bg-white/[0.02]">
                        <p className="text-[8px] font-black uppercase tracking-widest text-black/40 dark:text-white/40">Total</p>
                        <p className="text-lg font-black text-surface-900 dark:text-white leading-tight">{squads.length}</p>
                    </div>
                    <div className="flex-1 rounded-xl border border-surface-200/50 bg-white/50 p-2 dark:border-white/[0.05] dark:bg-white/[0.02]">
                        <p className="text-[8px] font-black uppercase tracking-widest text-black/40 dark:text-white/40">Active</p>
                        <p className="text-lg font-black text-primary-600 leading-tight">{squads.filter(s => s.status === 'active').length}</p>
                    </div>
                </div>
            </div>
            ...
            {loading ? (
                    ...
                    {filteredSquads.map((squad, index) => (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={squad._id}>
                            ...
                            >
                                ...
                                <h3 className="text-sm font-bold text-surface-900 dark:text-white group-hover:text-primary-600 transition-colors uppercase tracking-tight">{squad.title}</h3>
                                <p className="mt-1 line-clamp-1 text-[11px] font-bold text-black/60 dark:text-white/50 uppercase tracking-tight">{squad.description || "No description."}</p>
                                <div className="mt-4 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="flex items-center gap-1 text-[9px] font-black uppercase text-black/50 dark:text-white/50">
                ...
                <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-surface-200 p-8 text-center dark:border-white/10">
                    <h3 className="text-sm font-bold text-surface-900 dark:text-white">No squads found</h3>
                    <Link href="/dashboard/squads/new" className="mt-4 rounded-lg bg-surface-900 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white dark:bg-white dark:text-black">Get Started</Link>
                </div>
            )}
        </div>
    );
